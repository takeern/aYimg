...
let path=require('path')//资源路径，符合web服务器路由约定即可

let server = http.createServer(function (request, response) {
    //request用来接收客服端数据，response用来向客服端发送服务器数据
  let pathname = url.parse(request.url).pathname//解析路径名
  pathname=decodeURI(pathname)//路径解码--可能隐式编码
  let realPath = path.join(__dirname, pathname)//实际路径--绝对路径
  let ext = path.extname(realPath)//.htmlJ
